			...
			<tbody>
				...
				if($rlist !== ''):
					...
					foreach($rlist as $row): 
						...
						$buyxtakey_trxno = ($row['buyxtakey_trxno']);
						$dis = ($row['is_approved'] == 'N' ? "disabled" : '');
						
						?>
						<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
							<td class="text-center" nowrap>
								<?=anchor('me-buyxtakey-vw/?buyxtakey_trxno=' . $buyxtakey_trxno, '<i class="bi bi bi-pencil" style="background-color:#167F92; color:#fff; padding: 3px 5px 3px;; border-radius: 5px;"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
								
							</td>
							<td nowrap><?=$row['buyxtakey_trxno'];?></td>
							...
							<td>
								<button class="btn btn-success btn-xs" onclick="javascript:__mbtn_buyxtakey_bdownload('<?=$row['buyxtakey_trxno'];?>');" <?=$dis;?>> <i class="bi bi-print"></i> Download</button>
							</td>
...
<script type="text/javascript"> 
	function __myredirected_rsearch(mobj) { 
		try { 
			...
			jQuery.ajax({ // default declaration of ajax parameters
				type: "POST",
				url: '<?=site_url();?>me-buyxtakey-recs',
    ...
    function __mbtn_buyxtakey_bdownload(buyxtakey_trxno){
    	try { 
    		__mysys_apps.mepreloader('mepreloaderme',true);
    		
    		var mparam = {
    			buyxtakey_trxno: buyxtakey_trxno

    		}; 
                   //console.log(mparam);
                  jQuery.ajax({ // default declaration of ajax parameters
                  	type: "POST",
                  	url: '<?=site_url();?>me-buyxtakey-barcode-dl',
                  	context: document.body,
                  	data: eval(mparam),
                  	global: false,
                  	cache: false,
